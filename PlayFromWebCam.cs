...
public class PlayFromWebCam : MonoBehaviour {
	...
	void Update ()
	{
		...
		// Sample RGB channels for every odd pixel in the 5x5 grid with ax and ay in the center
		...
		// INTENSITY
			...
			if (vv<=0.6f) {intensityDetect = "Dark";}
			else
			{
				if (ss<=0.5f && vv>=0.75f) {intensityDetect = "Light";} else {intensityDetect = "";}
			}


		if (ss<=0.25f && vv>=0.75f) {colorReport="White"; intensityDetect = "";}
		if ((vv<=0.35f && ss<=0.35f) || vv<=0.25f) {colorReport="Black"; intensityDetect = "";}

    	//buttonColorInfo.GetComponentInChildren<Text>().text = "H: " + hueDetect.ToString("F1") + " S: "+ss.ToString("F2") + " V:" + vv.ToString("F2") + " = " + intensityDetect + " " + colorReport;
    ...
    {
        if (_buttonPressed == "Terms of Use") {showTerms();}
    }
    ...
    public void sliderSwitch()
    {
    	...
    	if (sliderVisible) {sliderSettings.interactable = true; GameObject.Find("ButtonSettings").GetComponent<Image>().sprite = checkImage; sliderSettings.transform.position = new Vector3(buttonSettings.transform.position.x + Screen.width/9.6f - buttonSpacing, buttonSettings.transform.position.y + Screen.width/4.8f + buttonSpacing, 0);}
    		else {sliderSettings.interactable = false; GameObject.Find("ButtonSettings").GetComponent<Image>().sprite = originalImage; sliderSettings.transform.position = new Vector3(-500, 0, 1);}
    }
